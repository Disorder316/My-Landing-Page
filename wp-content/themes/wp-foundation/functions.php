...
/*
        ...
        Author: Steve Withington (http://stephenwithington.com)
...
function get_logo($type='') {
    ...
    switch($type) {
        case 'mobile':
            $logo = $base . '/images/hutchens/hutch-logo-mobile.png';
            break;
        default:
            $logo = in_array('home', $class) 
                ? $base . '/images/hutchens/hutchens-logo-home.png'
                : $base . '/images/hutchens/hutchens-logo.png';
    }
    return $logo;
} 
...
function load_ljq() {  
  ...
  wp_register_script(
    $handle='jquery'
    , $src=get_bloginfo('template_url').'/javascripts/jquery.min.js'
    , $deps=__FILE__
    , $ver='1.7.2'
    , $in_footer=false
...
add_action('wp_enqueue_scripts', 'load_ljq'); // initiate the function  
...
function bones_register_sidebars() {
    ...
    register_sidebar(array(
    	'id' => 'sidebar3',
    	'name' => 'Form Sidebar',
    	'description' => 'Used only on the left sidebar form page template.',
    	'before_widget' => '<div id="%1$s" class="widget %2$s">',
    	'after_widget' => '</div>',
    	'before_title' => '<h4 class="widgettitle">',
    	'after_title' => '</h4>',
